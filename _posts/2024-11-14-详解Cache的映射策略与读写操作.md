...
- 关联映射（Associative Mapping）指的是主存中的所有Block都可以被载入到任意Cache行中
- 为这种映射设计的主存地址编码结构如下
...
### 2.4 组相联映射
- 分组关联映射（Set Associative Mapping）是一种混合了直接映射与关联映射的映射策略，这个策略将Cache的Line划分为若干组，每一个Block会被映射到唯一一个Line的组内，其可以被放到该组内的任意一行
