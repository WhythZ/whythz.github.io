...
#### 2.2.4 创建分离匿名线程
- 通过先创建临时的匿名线程，然后使用`detach()`将其与主线程分离（若不分离，匿名线程会直接被销毁，这会导致程序崩溃）
	- 分离线程在后台独立运行，匿名使得其无法通过`join()`方法阻塞主线程，执行完毕自动释放资源（关于`join()`和`detach(0`的使用详见后文）
	- 主线程不再管理该线程的生命周期，若主线程退出，所有分离线程会被强制终止，这有可能导致资源泄漏（故而需要在使用时注意内存的管理）
...
- 我学习到这时存在一个认识误区，即错误地认为：~~线程在调用了`join()`或`detach()`后才会开始执行，主线程会等待`_thread01`执行完后才输出`"Flag"`，然后才执行`_thread02.join()`~~，实际上线程一旦被创建就开始了执行，这就导致了
	- 由于两个线程是并发执行的，虽然`join()`方法会阻塞主线程使其等待子线程执行完后再继续执行，但子线程相互之间并不会相互阻塞等待，而是并发执行`Func01`和`Func02`，导致输出的`"1"`和`"2"`是交错在一起的
- 在看了上述程序的输出结果后，我疑惑为何输出的`"Flag"`处于所有的`"1"`和`"2"`之后
