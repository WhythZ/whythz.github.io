...
### 2.6 程序的中断
- 程序执行过程中有时会遇到需要进行中断（Halt）的情况，在处理器接收到中断信号后，必须得执行完当前指令才能结束程序
...
#### 4.2.1 硬布线控制介绍
- 硬布线控制（Hardwired Control）产生信号的思路总体如下图所示，其中Step Counter产生的五个$T_i$信号都是$1$位的控制信号，$T_i$可用于与其它条件做与运算以计算出某些信号，使其产生必须发生在对应的第$i$步
...
#### 4.2.5 微程序控制介绍
- 微程序控制（Microprogramming）产生控制信号的过程中，每个时钟周期内的每个控制信号值都是不同的，这些值都被存储在叫做微指令（Microinstruction/Control Word）的二进制编码中

![微程序控制产生控制信号的过程.png](/resources/组成原理/微程序控制产生控制信号的过程.png)

- 在这种方式的实现下，一条处理器指令的执行过程就被实现为：执行存放于Control Store的==多个微指令组成的序列==（称为Microprogram或Microroutine）
- 所以Control Circuitry解码IR代表的指令的过程实际上就是产生Microprogram的过程（下图中的$\mu PC$就是一个存放当前需要被用于执行的Microprogram所在位置的地址）
